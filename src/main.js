...
// ==========================================================================
// Language Detection (Multilingual SEO)
// ==========================================================================
...
// ==========================================================================
// Default Sample Markdown (English)
// ==========================================================================
const sampleMarkdownEn = `# [Transform Your Docs] Is Your Markdown Hard to Read? Design Tips to Polish Your Text Instantly

Do you write in **Markdown** for Notion, blogs, or internal docs but feel like it looks cluttered and uninviting?
Often, default Markdown rendering lacks proper vertical rhythm and visual cues, leading to poor user experience, high bounce rates, and drop-offs.

With **Markdown Viewer**, you can adjust fonts, sizes, line heights, margins, and header styles using simple sliders to transform your plain text into stunning, highly-readable documents.

---

## 📝 What is Markdown? Basics and Evolution

### The Philosophy Behind Markdown
Markdown is a **lightweight markup language** created in 2004 by John Gruber and Aaron Swartz. Their goal was to create a syntax that is "easy-to-read and easy-to-write in plain text format, which optionally converts to structurally valid XHTML (or HTML)."

Today, Markdown is the industry standard for document formatting across developer-centric platforms:
* **Notion**: Document organization and project wikis.
* **GitHub**: Project documentation using \`README.md\`.
* **Zenn / Medium / Dev.to**: Technical writing and developer blogs.
* **Slack / Discord**: Inline code snippets and basic text formatting.

### 5 Game-Changing Benefits of Markdown
1. **Mouse-Free Formatting**: Apply headers, bold text, or lists instantly using symbols (\`#\`, \`*\`, \`-\`) without touching your mouse.
2. **Universal Compatibility**: Since it's plain text, you can open and edit \`.md\` files on any OS (Mac, Windows, Linux, iOS, Android) or text editor.
3. **Seamless HTML Generation**: Instantly compiles to clean HTML, making it perfect for blogging platforms and static sites.
4. **Distraction-Free Writing**: Focus purely on your words and structured thought, rather than getting bogged down by style formatting.
5. **Human-Readable Source**: Unlike raw HTML, Markdown source code is clean and intuitive to read even without a parser.

---

## 🤖 Markdown in the Age of Generative AI, LLMs & Modern IDEs

In the era of AI-assisted programming (GitHub Copilot, Cursor, Windsurf) and advanced LLMs (ChatGPT, Claude, Gemini), Markdown has become an **essential skill** for prompt engineering and context optimization.

### 1. Structured Prompting for High-Quality Output
When writing complex prompts or System Instructions, using Markdown headers, lists, and blockquotes helps LLMs parse instructions, parameters, and input contexts accurately. This structure significantly improves the precision of the AI's response.

### 2. Token Optimization & Cost Reduction (RAG)
Markdown is far more compact than JSON, XML, or HTML. When feeding context documents into an LLM (e.g., in a RAG pipeline), using Markdown minimizes token usage, saving API costs and maximizing the model's active context window.

### 3. Native IDE Integration
Modern AI-powered editors like **Cursor**, **Windsurf**, and **VS Code** use Markdown as their primary format for documentation, inline chats, and developer notes. Having a fast, customizable viewer helps developers visualize documentation right in their workspace.

---

## 🚀 Quick Markdown Cheat Sheet

Here are the most common syntax elements you will use daily:

### 1. Headers
Use \`#\` followed by a space at the start of a line.
...
| Feature | Support | Key Benefit |
|---|---|---|
| Sliders | Real-time | Intuitive design |
| Export | Copy CSS | Ready for production |

---

## 💻 Online Web App vs Chrome Extensions

Unlike browser extensions like Chrome's "Markdown Viewer", which require extra file permissions and manual installations, this **Markdown Viewer (markdown-viewer.net)** is a fully web-based app:

* **Zero Installation**: Access via URL and start immediately.
* **Drag-and-Drop**: Import your local \`.md\` files instantly by dragging them into the browser.
* **Full Styling Customization**: Go beyond simple theme templates and fine-tune margins, fonts, and headers with sliders.
* **Responsive Layouts**: Check your document styles on desktop, tablet, and mobile browsers seamlessly.

---

## 🚀 3 Golden Rules for Highly Readable Documents

Use this viewer to easily implement the following typography best practices:

* **Font Size & Line Height**: Set paragraph size to \`16px\` and line-height to \`1.8\` for the most comfortable reading experience.
* **Visual Rhythm**: Add a clean border bottom below \`H1\` or \`H2\` elements to clearly demarcate document sections.
* **Generous Margins**: Ensure you leave \`16px - 24px\` of bottom margin below paragraphs and headers to prevent eyes from skipping lines.

\\\`\\\`\\\`css
/* Example of polished Markdown typography */
.markdown-body h1 {
  font-size: 28px;
  color: #6366f1;
  border-bottom: 3px solid #6366f1;
...
> **💡 Quick Tip**:
> High readability builds trust and increases time on site. Play around with the style settings to find your perfect layout!
`;

const sampleMarkdown = isEn ? sampleMarkdownEn : sampleMarkdownJa;
...
function bindEvents() {
  ...
  document.getElementById('btn-clear-editor').addEventListener('click', () => {
    if (editor.value.trim() === '') {
      showToast(isEn ? 'Already empty' : 'すでに空です');
      return;
    }
    ...
    renderMarkdown();
    saveState();
    showToast(isEn ? 'Markdown cleared' : 'マークダウンをクリアしました');
  });
  ...
  document.getElementById('btn-reset-styles').addEventListener('click', () => {
    ...
    showToast(isEn ? 'Style configurations reset to defaults!' : 'スタイル設定を初期値にリセットしました！');
  });
  ...
  themeToggle.addEventListener('click', () => {
    ...
    updateControlsUI();
    saveState();
    showToast(isEn ? `Switched to ${appTheme === 'dark-theme' ? 'dark' : 'light'} mode` : `${appTheme === 'dark-theme' ? 'ダーク' : 'ライト'}モードに切り替えました`);
  });
  ...
  document.getElementById('btn-copy-css').addEventListener('click', () => {
    ...
    navigator.clipboard.writeText(css).then(() => {
      ...
      showToast(isEn ? 'Failed to copy' : 'コピーに失敗しました');
    });
  });
  ...
  overlay.addEventListener('drop', (e) => {
    ...
    if (files && files.length > 0) {
      ...
      if (extension === 'md' || extension === 'txt' || file.type.startsWith('text/')) {
        ...
        showToast(isEn ? 'Error: Only .md or .txt files are supported' : 'エラー: .md または .txt ファイルのみ対応しています');
      }
    }
  });
}
...
function initApp() {
  ...
  fontOptions.forEach(opt => {
    ...
    let name = opt.name;
    if (isEn) {
      if (opt.id === 'system-sans') name = 'System Sans-Serif';
      else if (opt.id === 'system-serif') name = 'System Serif';
    }
